...
function get20s(artistsArr){
  ...
  for (let i=0; i < artistsArr.length; i++){
    let yearsExtract = artistsArr[i].years.match(/(\d+)/); 
    if (yearsExtract[0] > 1900 && yearsExtract[1] < 2001){
     collected20s.push(artistsArr[i].name);
    };
  };
  return collected20s;
};
...
*/
function removeArtist(artistsArr, index) {
...
zombieArtistObj = {
  "id": 20,
  "name": "Zom Bie", 
  "years": "It's a secret - Not completely dead yet",
  "genre": "Comedic Horror", 
  "nationality": "Worldly Zombie",
  "bio": "Zombie ipsum reversus ab viral inferno, nam rick grimes malum cerebro. De carne lumbering animata corpora quaeritis. Summus brains sit​​, morbo vel maleficia?" /*Zombie ipsum*/
};

function addArtist(newArtistObj, artistArr){
  artistArr.push(newArtistObj);
  return artistArr;
  };

console.log(addArtist(zombieArtistObj,artists));
...
function lotsOfArt(artistsArr){
  ...
  for (let i=0; i < artistsArr.length; i++){
      if (artistsArr[i].paintings > 100){
        painted100Plus.push(artistsArr[i].name);
      }
  };
  return painted100Plus;
};

console.log(lotsOfArt(originalArtists));
